...
// JSON.stringify(m) of the object itself. The tuple fixes field order, so a
// stored entry (from Mongo) and a submitted one (rebuilt by the browser)
// canonicalize the same despite differing key order. Encoding rather than
// joining removes delimiter injection: a `|`-joined form collided —
// {storageKey:"a", url:"b|c"} and {storageKey:"a|b", url:"c"} both produced
// "image|a|b|c|||", so an edit between them reported dirty:false and silently
// wrote NOTHING. `alt` is free-typed and `url` is format-unvalidated, so a
