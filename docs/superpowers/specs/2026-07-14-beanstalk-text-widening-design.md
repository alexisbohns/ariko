...
| Triage prefill | `buildVersionInput`: when the form's description (en **and** fr) is blank, carry `capture.body` **verbatim** (bilingual note survives promote); a typed form value wins as today. Name prefill from `capture.title` unchanged (titles are plain strings). |
...
- `buildVersionInput`/`buildVersionPatch`: compose from paired fields; bilingual capture body
  carried verbatim when both description fields blank; typed value wins; validation accepts
  fr-only name and rejects fully-blank name.
