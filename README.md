...
`middleware.ts` gates `/admin/:path*` (everything but the login page) on the signed session cookie and redirects a failure to it; every server action in `app/admin/actions.ts` opens with `requireSession()` as well, because a POST passes through no navigation gate.

The chrome is rendered once by the layout, not by the pages, in three clusters: the Ariko mark and the **plant switcher** top-left, a floating icon rail on the left edge — Inbox, Pods, Beans, Sprouts, Screens, Beanstalk (`lib/admin-nav.ts`) — and the public-site and log-out buttons top-right. The ⌘K palette rides with it, so it works on every admin route. A section index reads in the wide column, every other route in the public site's own measure.

**The admin's subject is a plant, and the URL always says which one.** The switcher sets it; `lib/admin-scope.ts` reads it — the slug on `/admin/plant/[slug]`, `?plant=<slug>` everywhere else — and builds every href that changes it, so a scoped view stays shareable and the narrowing stays server-side, exactly as `lib/admin-filters.ts` already did for the row filters. Every row of the switcher is a plain `<a href>`: picking a plant is a navigation, never a write. Scoped, the rail gains **Overview** (the plant's own hub) and drops **Beanstalk**, which merges sprouts with pollen envelopes that have no plant and so cannot honestly be narrowed to one.

`/admin/garden` and `/admin/vault` no longer exist: plants moved to the welcome page, the pod half became `/admin/pods`, and the vault became `/admin/sprouts` — "Vault" named no tier, and each section is now named for the tier it lists. Both old addresses redirect (`next.config.ts`, `permanent: false` — these are for bookmarks, not promises).

Everything routeable except the login page lives under `app/admin/(chrome)/`. That is a privacy boundary, not tidiness: the chrome layout reads the garden to compose the switcher's marks and hands them to a client island, which serializes them into the HTML — so the one route `middleware.ts` lets through anonymously must never enter that layout. A route group's name is invisible to the URL, so no admin address changed. `lib/admin-login-layout-source.test.ts` pins it.
...
The login gate, and the one admin route that carries no chrome — it sits outside `app/admin/(chrome)/` rather than asking the chrome to withdraw. Submitting the correct `ADMIN_PASSWORD` sets an httpOnly, `SameSite=Lax` (Secure in production) signed cookie good for 30 days and redirects to `/admin`; a wrong password re-renders with an error. Either secret unset means every password is wrong — the door fails closed.

### `/admin` — the welcome page

A greeting, the ⌘K autocomplete inline and auto-focused, and every plant in two tables — **Published** and **Private**, each with its count, each the same component rendered twice so they cannot drift. Columns: name · role · status · narrative. There is no tier column because only plants are listed, and no visibility column because the two tables *are* the visibility. A row's name is that plant's hub.

The search is not a second palette: `command-palette.tsx` splits into `PaletteAutocomplete` (the input, the `/admin/palette` fetch, the grouped rows) and two shells around it — the chrome's ⌘K dialog and this page's bare one. Script-off the input is simply absent, and it adds no destination the tables below do not already offer.

### `/admin/inbox`

The `status:"inbox"` seeds, newest first: source glyph, title, note snippet, media count, age. A row's title is its triage page. Scoped to a plant, it narrows on `seed.suggested.plantSlug`; a seed suggesting nothing, or naming a plant the garden no longer holds, appears under **All** and under no plant — the same rule the source column already follows when it declines to draw an avatar for a suggestion it cannot resolve. Seeds arrive from the sibling repos' CIs through `POST /api/inbox` (§Ingestion spine) and from the capture overlay here — the `+` beside the title, or the `k` key, opens a full-screen sheet with an autofocused title, a note with an en/fr toggle, paste-a-link fields and the media picker in its compact form, posting to the same `createSeedAction` the API path ends in. The overlay is handed the **unfiltered** seed total, because that count is its only signal that a save landed: under a scope, a capture whose suggestion falls outside the current plant would leave the narrowed count unchanged and the overlay sitting open over a seed that was in fact saved (`lib/admin-section-source.test.ts`). The garden is loaded separately from the seeds and fails separately: an unreachable garden costs the source column's plant avatars, never the list.
...
Every pod: name, plant, bean count, visibility, narrative. Narrowing compares the plant refs in `parents` and a pod matches on **any** of them, the way `Dataset.podsForPlant` answers — so a pod parented to two plants appears under both, and a hub's count and this section's rows agree. The plant column draws the first parent that resolves; a pod whose plant refs all dangle draws an em dash rather than filing itself under a plant the garden no longer holds.

### `/admin/beans`

Every bean — the index beans never had: name, plant, pod, sprout count, visibility. Scoped, it narrows through `beansForPlantDeep` (`lib/plant-hub.ts`): a plant's beans are its direct ones **plus** every one of its pods'. One function rather than two pieces of arithmetic that agree today, which is what stops a hub preview saying 4 over a section that shows 11 (`lib/admin-section-source.test.ts`).

### `/admin/sprouts`

Every sprout in the archive regardless of state or visibility — the authoring counterpart to the public `/beanstalk`. Sprout, state, plant mark, bean, date and tags, newest first, filtered by `state` / `plant` / `tag`. The filter popovers contain nothing but `<a href>` query-param links (`lib/admin-filters.ts`), so filtering stays server-side in `lib/sprouts.ts` and a filtered URL is shareable; `s`, `p` and `t` open them. A row's name leads to its bean page.

The four tier sections draw their rows from one file each — `plant-table.tsx`, `pod-table.tsx`, `bean-table.tsx`, `sprout-table.tsx`, over a shared `table-cells.tsx` — and a plant hub's previews render those very components with a row limit inside `preview-panel.tsx`. Same file, different parameter: the shared-surfaces rule applied inside one zone. All six are **server** components, and `lib/admin-table-source.test.ts` is what says so, because adding `"use client"` to one to wire up a sort header would pass `tsc`, `npm test` and `npm run build` while shipping the table and everything it imports to the browser.
...
* **Plant** — the mark centred in a squircle with the name beneath it, and each editor one click behind the thing it edits: the logo behind the logo, name and description behind the title, the role behind a crown, status and visibility behind two icons that open their vocabulary as radios and commit on a separate Save (disabled until the pick differs from what is stored — a stray click should not unpublish a project). Below that is the narrative, unboxed. **The page is a hub as well as an editor**: under the prose sit four previews — Pods, Beans, Sprouts, Screens — each a heading with a count, a few rows (five, or four thumbnails for screens), and an `all n →` link into that section carrying `?plant=`. The first three render the sections' own table components; the screens preview is plain anchors around thumbnails rather than the library's tiles, which are this repo's only `next/link` and stay confined to the four files of the interception slice. What remains on the right-hand rail is the **exhibition** alone, with ↑ / ↓ / ✕ per row, because re-sequencing a strip is the one thing a preview cannot do.

  One honest gap: the screens count matches **any** plant parent, while `/admin/screens?plant=` narrows on the first (`screenRows` in `lib/screens.ts`), so a screen parented to two plants is counted on both hubs and listed under one, and `all n →` would land on fewer rows than the heading promised. No screen in the garden has two plant parents today and the create form cannot make one; if that changes, both sides have to be made to agree rather than one side papered over.
...
A contact sheet rather than a table. A hundred and seventy rows of `match-hero-m104-final-spain-argentina-aet` are not scannable by eye, and those names are honest filename stems because nobody was ever going to write a hundred and seventy titles — so the page shows the screens themselves, `object-contain` (a 9:19.5 capture cropped to a tile is a picture of its middle third), with the name as the caption and the sprouts section's filter bar carrying `plant` / `bean` / `tag`. `npm run import:screens` fills it; the `+` beside the title is a link to the create page.

A screen's own page carries its metadata, its exhibition — which plant's strip it appears on — its image and a delete, with prev/next walking the *filtered* set. Clicking a tile opens that page in a panel on the right that the index slides out from under: `app/admin/(chrome)/@sheet/(.)screens/[slug]`, a parallel + intercepting route that **imports and renders the page's own module** rather than reimplementing it. That is the whole design — there is nothing in the panel that is not a page, so script-off the same click is an ordinary navigation to the same editors, and prev, next and close stay real `<a href>`s. `lib/screen-sheet-source.test.ts` pins both halves. Two files there look like dead code and are load-bearing: `@sheet/default.tsx` returns null, which is what makes the layout's `:has(~ [data-screen-sheet])` push honest on every other route, and `@sheet/screens/page.tsx` gives the slot a route that matches the library itself — without it a soft navigation keeps the slot's last state and the panel stays open over the grid it just returned to.
